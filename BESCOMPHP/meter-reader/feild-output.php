...
<div class="space-y-6">
    ...
    <!-- Sub-header banner -->
    <div class="bg-primary/95 text-white font-bold text-center py-2.5 rounded-xl uppercase tracking-wider text-sm shadow">
        AI Meter Reading Output
    </div>
        ...
        <form id="filter-form" class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select id="mode-select" class="w-full border border-slate-300 rounded-lg px-3 py-1.5 text-xs focus:ring-1 focus:ring-primary focus:border-primary focus:outline-none bg-white">
                        <option value="All">All</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="mr-id-input" placeholder="Enter MR ID" class="w-full border border-slate-300 rounded-lg px-3 py-1.5 text-xs focus:ring-1 focus:ring-primary focus:border-primary focus:outline-none bg-white">
                </div>
                <div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <!-- Badges -->
            <span class="bg-white border border-primary/20 text-slate-800 text-[10px] font-bold px-2.5 py-1.5 rounded shadow-sm">
                Total Count: <strong id="total-count-badge" class="text-primary">0</strong>
            </span>
            <span class="bg-white border border-sky-200 text-slate-800 text-[10px] font-bold px-2.5 py-1.5 rounded shadow-sm">
                OCR Count: <strong id="ocr-count-badge" class="text-sky-600">0</strong>
            </span>
            <span class="bg-white border border-rose-200 text-slate-800 text-[10px] font-bold px-2.5 py-1.5 rounded shadow-sm">
                OCR Corrected: <strong class="text-rose-600">0</strong>
            </span>
        </div>
    ...
    <!-- Table Container (matches 21 columns with double borders/grid layout) -->
    <div class="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
        <div class="overflow-x-auto w-full">
            <table class="w-full text-center border-collapse border border-slate-300 min-w-[2000px]">
                <thead>
                    <tr class="bg-slate-100 text-slate-700 text-xs font-bold uppercase border-b border-slate-300">
                        <th class="py-3 px-2.5 border border-slate-300">Sno.</th>
                        <th class="py-3 px-2.5 border border-slate-300">Date</th>
                        <th class="py-3 px-2.5 border border-slate-300">Time</th>
                        <th class="py-3 px-2.5 border border-slate-300">Request ID</th>
                        <th class="py-3 px-2.5 border border-slate-300">Account ID</th>
                        <th class="py-3 px-2.5 border border-slate-300">Meter Number</th>
                        <th class="py-3 px-2.5 border border-slate-300">Meter Model</th>
                        <th class="py-3 px-2.5 border border-slate-300">Meter Reading (KWH)</th>
                        <th class="py-3 px-2.5 border border-slate-300">MD (KW)</th>
                        <th class="py-3 px-2.5 border border-slate-300">Meter Image (KWH)</th>
                        <th class="py-3 px-2.5 border border-slate-300">Image Path (KWH)</th>
                        <th class="py-3 px-2.5 border border-slate-300">Meter Image (KW)</th>
                        <th class="py-3 px-2.5 border border-slate-300">Image Path (KW)</th>
                        <th class="py-3 px-2.5 border border-slate-300">Latitude Longitude</th>
                        <th class="py-3 px-2.5 border border-slate-300">PF</th>
                        <th class="py-3 px-2.5 border border-slate-300">Meter Image (PF)</th>
                        <th class="py-3 px-2.5 border border-slate-300">Image Path (PF)</th>
                        <th class="py-3 px-2.5 border border-slate-300">Address</th>
                        <th class="py-3 px-2.5 border border-slate-300">OCR STATUS</th>
                        <th class="py-3 px-2.5 border border-slate-300">MODE</th>
                        <th class="py-3 px-2.5 border border-slate-300">MR ID</th>
                    </tr>
                </thead>
                <tbody id="table-data-body" class="divide-y divide-slate-200 text-xs font-semibold text-slate-700 bg-white">
                    <!-- Empty State placeholder -->
                    <tr id="empty-state-row">
                        <td colspan="21" class="py-12 text-center text-slate-400 font-bold text-sm bg-slate-50">
...
<script>
    ...
    function applyFilters() {
        ...
        rows.forEach(row => {
            ...
            if (modeFilter !== 'All' && mode !== modeFilter) matches = false;
            ...
            if (matches) {
                ...
                visibleCount++;
                if (ocr === 'SUCCESS') ocrSuccessCount++;
            } else {
        ...
        document.getElementById('ocr-count-badge').textContent = ocrSuccessCount;
    }
    ...
    function resetFilters() {
        ...
        document.getElementById('ocr-count-badge').textContent = '0';
    }
